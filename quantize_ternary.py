...
def quantize_ternary(weight: torch.Tensor, group_size: int = 128):
    """
    Quantize FP16 weights to ternary {-1, 0, +1} using per-group absmean.

    For each group of group_size consecutive weights along the K axis:
        gamma = mean(|W_group|)
    ...
    Returns:
        ...
        scales:    FP16 tensor of shape (N, K // group_size) holding gamma per group.
    """
    N, K = weight.shape
    assert K % group_size == 0, f"K={K} must be divisible by group_size={group_size}"

    weight_groups = weight.reshape(N, K // group_size, group_size).float()
    # BitNet's absmean: gamma = mean(|W|) per group
    ...
    return quantized.reshape(N, K), gamma.to(torch.float16)
